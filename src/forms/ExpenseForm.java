 ...
 */
public class ExpenseForm extends SimpleForm {
    ...
    private void loadExpenseData() {
        try {
            ...
            // Check if balance is negative and display warning message
            if (balanceAmount < 0) {
                JOptionPane.showMessageDialog(this, "High Spending Alert: You're spending beyond your income. Please ensure better management of your finances.", "Warning", JOptionPane.WARNING_MESSAGE);
            }

        } catch (SQLException ex) {
            ex.printStackTrace();
            JOptionPane.showMessageDialog(this, "Error loading expense data", "Error", JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    private double calculateBalance() {
        ...
        try {
            // Fetch total income amount from the incomes table
            Connection conn = DriverManager.getConnection("jdbc:sqlite:expense.db");
            String incomeSql = "SELECT SUM(amount) FROM incomes";
            PreparedStatement incomePstmt = conn.prepareStatement(incomeSql);
            ResultSet incomeRs = incomePstmt.executeQuery();
            if (incomeRs.next()) {
                incomeAmount = incomeRs.getDouble(1);
            }
            incomeRs.close();
            incomePstmt.close();
    ...
    // Other methods and components of your Expense class...
    ...
    try {
        Connection conn = DriverManager.getConnection("jdbc:sqlite:expense.db");
        String sql = "INSERT INTO expenses (date, description, amount, category, month) VALUES (?, ?, ?, ?, ?)";
        PreparedStatement pstmt = conn.prepareStatement(sql);
        pstmt.setString(1, date);
        pstmt.setString(2, description);
        pstmt.setDouble(3, amount);
        pstmt.setString(4, category);
        pstmt.setString(5, month); // Store the extracted month
        pstmt.executeUpdate();
        pstmt.close();
        conn.close();
    } catch (SQLException ex) {
    ...
    // Reload expense data to update the table
    loadExpenseData();
} 
    ...
    try {
        Connection conn = DriverManager.getConnection("jdbc:sqlite:expense.db");
        String sql = "DELETE FROM expenses WHERE date = ? AND description = ? AND amount = ? AND category = ?";
        PreparedStatement pstmt = conn.prepareStatement(sql);
        pstmt.setString(1, date);
        pstmt.setString(2, description);
        pstmt.setDouble(3, amount);
